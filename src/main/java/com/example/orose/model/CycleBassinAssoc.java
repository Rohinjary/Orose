...
private Boolean estCloture = false;
    ...
    // ------------------------------------------------------------
    // Champ calculé (non persisté) : semaine calendaire réelle,
    // basée sur la date du jour — recalculée à chaque appel.
    // Convention : S0 = les 7 premiers jours du cycle.
    // Différent de "semaineActuelle" qui reflète la semaine de
    // la DERNIÈRE PESÉE saisie (utilisée pour comparer aux courbes
    // de croissance standard dans BiologiqueService).
    // Même convention de calcul que PeseeService via SemaineUtils,
    // pour éviter tout décalage entre les deux notions affichées
    // à l'utilisateur.
    // ------------------------------------------------------------
    @Transient
    public int getSemaineCalendaire() {
        if (cycle == null || cycle.getDateDebut() == null) {
            return 0;
        }

        LocalDate fin = cycle.getDateFinPrevue();
        LocalDate reference = (fin != null && LocalDate.now().isAfter(fin))
                ? fin
                : LocalDate.now();

        return SemaineUtils.calculer(cycle.getDateDebut(), reference);
    }
}